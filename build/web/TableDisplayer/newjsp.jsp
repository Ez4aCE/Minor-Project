...
<html>
    <head>
        ...
        <title>BOOKED train Record Displayer</title>
    ...
    <%!
        ...
        <%
            ...
            {              
                   ...
                   opt =(OraclePreparedStatement) oconn.prepareStatement("DELETE FROM BOOKEDTRAINS WHERE TICKETID = ?");
                   opt.setString(1, ticketid);
                   int x = opt.executeUpdate();
                   
                   if(x>0){
                   response.sendRedirect("http://localhost:8080/Bookinggo/TableDisplayer/newjsp.jsp");
                    return;
                   }
                   opt.close();
                   oconn.close();
        ...
        opt = (OraclePreparedStatement) oconn.prepareStatement("INSERT INTO BOOKEDTRAINS (TICKETID, ID, PASSNAME, CARDNUMBER, TRAINID, TRAVELDATE, FARE, SEATNO) VALUES (TICKETID_SEQ.NEXTVAL,?, ?, ?, ?,?,?,?)");
        opt.setInt(1, id);
        opt.setString(2, passname);
        opt.setInt(3, cardnumber);
        opt.setInt(4, trainid);
                ...
                <tr>
                    <%
                       for(counter = 1; counter <= orsmd.getColumnCount(); counter ++)
                        ...
                        %>
                        <td>
    <form method="post" action="http://localhost:8080/Bookinggo/TableDisplayer/newjsp.jsp"> 
        ...
        <button type="submit" class="btn btn-danger" name="bConfirm">delete</button>
            ...
            <td colspan="<%= orsmd.getColumnCount() + 1 %>">
                <p>enter the records: (!!!  IMPORTANT - PLZ TRY NOT TO ENTER RECORDS FROM HERE UNLESS SOME DATABASE ERROR AND USER PAYMENT IS DONE BUT NOT SHOWING HIS/HER DATA IN DATABASE.... ELSE USE THE DIRECT PAYMENT PORTAL TO BOOK !!!!
                    YOU WILL NEED REAL ID,NAME,CARDNUMBER OF THAT USER AND trainID WITH THAT DATE AND PRICE OR ELSE IT CAN CAUSE ERRORS)<p> 
                <form method="post" action="http://localhost:8080/Bookinggo/TableDisplayer/newjsp.jsp">
                        ...
                        <input type="number" name="trainid" placeholder="trainid" required>
                        <input type="date" name="date" placeholder="New date" required>
                        <input type="number" name="price" placeholder="New price" required>
                        <input type="number" name="no.seat" placeholder="seat number" required>
